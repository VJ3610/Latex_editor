function EqTextArea(preview, input, comment, download, intro) {
    ...
    this.updateExportArea = function () {
      for (i = 0; i < this.bsize; i++) {
        ...
        else if (this.bArray_area[i].value !== undefined)
          this.bArray_area[i].value = v;
        else if (this.bArray_area[i].innerHTML !== undefined)
          this.bArray_area[i].innerHTML = v;
      }
    };
    ...
    this.addEvent = function (action, fn) {
      if (this.equation_input.addEventListener)
        this.equation_input.addEventListener(action, fn, false);
    ...
    this.setText = function (val) {
      ...
      do {
        ...
        if (i != -1) {
          ...
          if (
            subtex.substr(0, 4) == "\\bg_" &&
            EqEditor.setSelIndx("bg", subtex.substr(4))
          )
            go = 1;
          if (
            subtex.substr(0, 4) == "\\fn_" &&
            EqEditor.setSelIndx("font", subtex.substr(4))
          )
            go = 1;
          if (
            subtex.substr(0, 5) == "\\dpi{" &&
            EqEditor.setSelIndx("dpi", subtex.substr(5))
          )
            go = 1;
    ...
    this.insertText = function (txt, pos, inspos) {
      ...
      if (document.selection) {
        ...
        while (
          theCaret.parentElement() == myField &&
        ...
        if ((leftbracket || insert_pos >= 0) && sel.text.length) {
          ...
          sel.text =
            txt.substring(0, ins_point) + sel.text + txt.substr(ins_point);
        } else sel.text = txt;
        ...
        case "wp":
          {
            ...
            return EqEditor.get_inline_wrap(
              this.getLaTeX(),
              "[latex]{$TEXT}[/latex]\n",
              "$latex {$TEXT}$ "
            );
    ...
    this.renderEqn = function (callback) {
      ...
      if (bracket == 0) {
        if (EqEditor.$("renderbutton"))
          EqEditor.$("renderbutton").className = "greybutton";
            ...
            case "pdf":
              ...
              this.setdownload(
                '<a target="_blank" href="' +
                  ...
                  "/pdf.download?" +
                  sval +
                  '"><img src="images/pdf.jpg" width="30" height="30" align="middle" />Click here to Download Equation (PDF)</a>'
              );
              break;
          }
        ...
        if (bracket < 0)
          this.setcomment(
            "<br/><span class=\"orange\">You have more <strong>closed '}' brackets</strong> than open '{' brackets</span>"
          );
        else
          this.setcomment(
            "<br/><span class=\"orange\">You have more <strong>open '{' brackets</strong> than closed '}' brackets</span>"
          );
      }
      this.changed = false;
    };
    ...
    this.undo = function (box) {
      ...
      if (this.myRedo < this.myUndo) {
        this.myRedo++;
        if (this.myRedo == this.myUndo && EqEditor.$("undobutton"))
          EqEditor.$("undobutton").src =
            EDITOR_SRC + "/images/buttons/undo-x.gif";
    ...
    this.redo = function (box) {
      if (this.myRedo > 0) {
        this.myRedo--;
        if (this.myRedo == 0 && EqEditor.$("redobutton"))
          EqEditor.$("redobutton").src =
            EDITOR_SRC + "/images/buttons/redo-x.gif";
  ...
  var EqEditor = {
    ...
    Example: {
        ...
        if (text != "") {
          this.load_json(
            ...
            "sid=" +
              EqEditor.SID +
              "&add&eqn=" +
              escape(text.replace(/\+/g, "@plus;").replace(/#/g, "@hash;"))
          );
      ...
      show: function (button, group) {
        ...
        if (group == "fav" || group == "history") {
          var d = new Date();
          EqEditor.Gallery.init(
            "photos",
            "leftarrow",
            "rightarrow",
            "overview",
            FAVORITE_ENGINE +
              "/example_json.php?fn=EqEditor.Gallery&rand=" +
              d.getTime() +
              "&sid=" +
              EqEditor.SID
          );
          ...
          EqEditor.Gallery.init(
            ...
            "rightarrow",
            "overview",
            FAVORITE_ENGINE + "/example_json.php?fn=EqEditor.Gallery"
          );
    ...
    Opacity: {
      ...
      fade: function (id, opacStart, opacEnd, millisec) {
        ...
        for (i = 1; i < count; i++)
          setTimeout(
            "EqEditor.Opacity.set('" + id + "'," + (i * sgn + opacStart) + ")",
            i * speed
          );
      },
      fadeout: function (id) {
    ...
    Cookie: {
      ...
      set: function (c_name, value, expiredays) {
        ...
        if (
          ...
          SID ||
          confirm(
            "This site uses cookie to remember your preference. Are you ok with this?"
          )
        ) {
          var exdate = new Date();
          ...
          document.cookie =
            ...
            (expiredays == null
              ...
              : ";SameSite=None; Secure;expires=" + exdate.toGMTString());
          document.cookie = "eqeditor_cookies=1;SameSite=None; Secure";
        }
      },
      get: function (c_name) {
      ...
      add: function (textarea, button_id, targetFn, mode) {
        ...
        if (a) {
          ...
          a.onclick = function (e) {
            var i = this.exportid;
            EqEditor.ExportButton.bArray_area[i].Export(
              EqEditor.ExportButton.bArray_fn[i],
              EqEditor.ExportButton.bArray_mode[i]
            );
            window.close();
          };
    ...
    add: function (obj, resize) {
      ...
      obj.equation_input.onfocus = new Function(
    ...
    embed: function (id, SID, design, language) {
      ...
      if (this.editor_id != id) {
        this.editor_id = id;
        var url =
          EMBED_ENGINE + "?id=" + id + "&SID=" + SID + "&design=" + design;
    ...
    backCursor: function (myField) {
      if (document.selection) {
        ...
        myField.value =
          myField.value.substring(0, s) +
          myField.value.substring(e + 1, myField.value.length);
    ...
    resize: function (num) {
      ...
      this.targetArray[num].equation_input.style.height =
        parseInt(Math.max((y - 200) / 3, 40)) + "px";
    },
  };
  ...
  Scroll.prototype = {
    ...
    add_panel: function () {
      if (this.ajax_php && this.ajax_response_fn) {
        if (this.ajax_php.indexOf("?") == -1)
          loadXMLDoc(
            this.ajax_php + "?panel=" + this.panels + this.subtext,
    ...
    redraw: function () {
      ...
      if (this.ajax_php && this.ajax_response_fn) {
        ...
        if (this.ajax_php.indexOf("?") == -1)
          loadXMLDoc(
            obj.ajax_php + "?panel=" + this.panels + this.subtext,
            obj.ajax_response_fn
          );
        else
          loadXMLDoc(
            obj.ajax_php + "&panel=" + this.panels + this.subtext,
    ...
    setarrow: function () {
      this.left_arrow.src =
        ...
        (this.visible <= 0 ? "leftarrow_grey.gif" : "leftarrow.gif");
